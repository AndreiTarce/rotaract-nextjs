...
import {
    ROTARIAN_YEAR_END_DATE,
    ROTARIAN_YEAR_START_DATE,
} from '@/lib/constants';
...
export async function getMemberAttendance(
    ...
    const period: IPeriod = {
        startDate: start_date
            ? new Date(start_date)
            : currentRotarianYear.startDate,
        endDate: end_date ? new Date(end_date) : currentRotarianYear.endDate,
    };

    const numberOfMeetings = await meetingInteractor.getNumberOfMeetings(
        type,
        period
    );

    const presences = await meetingInteractor.getMemberPresences(
        memberId,
        type,
        period
    );
