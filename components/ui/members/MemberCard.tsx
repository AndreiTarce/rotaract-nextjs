...
const MemberCard = (props: MemberDto) => {
    const personalLinks = (
        <>
            {props.urls?.facebook && (
                <Link href={props.urls.facebook} className="w-fit" target="_blank">
                    <FontAwesomeIcon icon={faFacebook} size="2x" />
                    <span className="sr-only">Facebook profile</span>
                </Link>
            )}
            {props.urls?.linkedin && (
                <Link href={props.urls.linkedin} className="w-fit" target="_blank">
                    <FontAwesomeIcon icon={faLinkedin} size="2x" />
                    <span className="sr-only">Linkedin profile</span>
                </Link>
            )}
            {props.urls?.instagram && (
                <Link href={props.urls.instagram} className="w-fit" target="_blank">
                    <FontAwesomeIcon icon={faInstagram} size="2x" />
                    <span className="sr-only">Instagram profile</span>
                </Link>
            )}
            {props.urls?.tiktok && (
                <Link href={props.urls.tiktok} className="w-fit" target="_blank">
                    <FontAwesomeIcon icon={faTiktok} size="2x" />
                    <span className="sr-only">Tiktok profile</span>
                </Link>
            )}
            {props.urls?.website && (
                <Link href={props.urls.website} className="w-fit" target="_blank">
        ...
        <Card className="flex max-w-full flex-[1_0_300px] flex-col overflow-hidden rounded-lg border shadow-md">
            ...
            <div className="flex h-fit flex-col p-5">
                <div>
                    ...
                    <p className="mb-3 flex flex-col gap-2 font-normal text-gray-700 capitalize dark:text-gray-500">
                        {props.role}
                        {(props.isBoard || props.role === memberRoles.PAST_PRESIDENT) && (
                            <Badge className="w-fit text-[12px] font-semibold">
                                {props.start_mandate} - {props.start_mandate + 1}
                            </Badge>
                        )}
                    </p>
